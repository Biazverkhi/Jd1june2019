...
public class Task2_getArrayNominalAmount {
    public static int[][] getNominal(int money, int[][] baseBankomat, int[][] moneyBankomat) {//функция возвращает массив с количеством купюр по номиналу, которые надо выдать
...
//расчет количества номиналов из следующих принципов: 20 до 4 штук, иначе 100. 50 - 1 шт иначе 1х100ка.
            ...
            if (moneyBankomat[0][0] >= array[0][0] + 1) {
    ...
    public static int[][] nominalDistribution(int money, int[][] array, int[][] baseBankomat, int[][] moneyBankomat) {
        ...
        if (moneyBankomat[2][0] >= count / 100)//если количество 100 в банкомате достаточно, то утверждаем значение
        {
            array[2][0] = count / 100;
            array[1][0] = count % 100 / 50;
        } //в массив добавляем сотни
        else {// если соток не хватает, то
            array[2][0] = moneyBankomat[2][0];//учитываем существующие сотки
            count -= array[2][0] * array[2][1];//отнимаем от счетчика учтенные в сотках деньги
            if (moneyBankomat[1][0] >= count / 50)//если 50 в банкомате хватает, то учитываем их.
            {
                array[1][0] = count / 50;
            }//в массив добавляем 50
            else {//если 50 не хватает, то
                array[1][0] = moneyBankomat[1][0] - moneyBankomat[1][0] % 2; //учитываем 50
                count -= array[1][0] * array[1][1];//отнимаем от счетчика учтенные в 50 деньги
                if (moneyBankomat[0][0] >= count / 20) {//если 20 в банкомате хватает, то
                    array[0][0] = count / 20;//учитываем их
                } else {
                    Task2_printOf.errorNoNominal(20);//если не хватает то ошибка
                }
            }
